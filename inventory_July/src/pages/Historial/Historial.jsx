...
export default function Historial() {  // Cambié el nombre de la función
  ...
  const cargarLogs = async () => {
    ...
    try {
      ...
      console.log('Datos del historial:', data) // Para depurar
      setLogs(data)
    } catch {
      setLogs([])
    } finally {
  ...
  // Filtrado correcto para los datos de historial
  const logsFiltrados = logs.filter(log => {
    const coincideBusqueda = 
      (log.nombre_producto?.toLowerCase().includes(busqueda.toLowerCase()) || false) ||
      (log.tipo_movimiento?.toLowerCase().includes(busqueda.toLowerCase()) || false) ||
      (log.cantidad?.toString().includes(busqueda) || false)

    const coincideAccion = filtroAccion === 'Todas' || log.tipo_movimiento === filtroAccion
  ...
  // Estadísticas corregidas - usamos los mismos valores que vienen del backend
   const estadisticas = {
    total: logs.length,
    ...
    <div className="his__page">
      ...
      <div className="his__header">
        <div>
          ...
          <p className="his__subtitulo">Registro de actividad y cambios en productos</p>
        </div>
      ...
      {/* Tabla - IMPORTANTE: pasar logsFiltrados, no logs */}
      <Table
        textBuscador="🔍 Buscar por producto, tipo o cantidad..."
        columnas={columnas}
        datos={logsFiltrados}  // ✅ Cambiado: antes era logs
        cargando={cargando}
        filtros={
          <select 
        ...
        >
          <div className='detalle-grid'>
            <div className="detalle-item">
              ...
              <span className="detalle-val">{formatFecha(logDetalle.fecha_movimiento)}</span>
            </div>
            
            <div className="detalle-item">
              <span className="detalle-label">Acción</span>
              <span className="badge" style={{
                background: COLORES_ACCION[logDetalle.tipo_movimiento]?.bg || '#f1f5f9',
                color: COLORES_ACCION[logDetalle.tipo_movimiento]?.color || '#475569'
              }}>
                {COLORES_ACCION[logDetalle.tipo_movimiento]?.label || logDetalle.tipo_movimiento}
              </span>
            </div>
            ...
            <div className="detalle-item">
              ...
              <span className="detalle-val">{logDetalle.id_venta ? `#${logDetalle.id_venta}` : '—'}</span>
            </div>
